...
describe("HTMLTextAdapter", () => {
  ...
  describe("lazy evaluation", () => {
    it("should not calculate value until getValue is called", () => {
      const textNode = parseText("<div>  hello  </div>");
      const adapter = new HTMLTextAdapter(textNode);

      expect(adapter._value).toBe(null);

      adapter.getValue();

      expect(adapter._value).toBe("hello");
    });

    it("should not calculate range until getRange is called", () => {
      const textNode = parseText("<div>  hello  </div>");
      const adapter = new HTMLTextAdapter(textNode);

      expect(adapter._range).toBe(null);

      adapter.getRange();

      expect(adapter._range).toEqual([7, 12]);
    });

    it("should not calculate location until getLocation is called", () => {
      const textNode = parseText("<div>  hello  </div>");
      const adapter = new HTMLTextAdapter(textNode);

      expect(adapter._loc).toBe(null);

      adapter.getLocation();

      expect(adapter._loc).toEqual({
        start: { line: 1, column: 7 },
        end: { line: 1, column: 12 },
      });
    });
    ...
    it("should calculate independently", () => {
      ...
      expect(adapter._range).not.toBe(null);
      expect(adapter._loc).toBe(null);
    });
  });
});
