...
ruleTester.run("no-ineffective-attrs", rule, {
  valid: [
    ...
    {
      code: '@if (isLoggedIn) { <input type="email" multiple /> }',
    },
    {
      code: '@for (item of items; track item.id) { <input type="file" accept=".jpg" /> }',
    },
  ],
  invalid: [
    {
    ...
    {
      code: '@if (show) { <input type="text" multiple /> }',
      errors: [
        {
          messageId: "ineffective",
          line: 1,
    ...
    {
      code: "@for (item of items; track item.id) { <a download>Link</a> }",
      errors: [
        {
          messageId: "ineffective",
          line: 1,
