...
ruleTester.run("no-obsolete-attrs", rule, {
  valid: [
    ...
    {
      code: "<custom-link charset='UTF-8'>Link</custom-link>",
    },
    {
      code: '@if (isLoggedIn) { <a href="/page">Link</a> }',
    },
    {
      code: '@for (item of items; track item.id) { <img src="{{item.url}}" alt="Image" /> }',
    },
    {
  ...
  invalid: [
    ...
    {
      code: '@if (show) { <div align="center">Content</div> }',
      errors: [
        {
          messageId: "obsolete",
          line: 1,
    ...
    {
      code: '@for (item of items; track item.id) { <table border="1"></table> }',
      errors: [
        {
          messageId: "obsolete",
          line: 1,
