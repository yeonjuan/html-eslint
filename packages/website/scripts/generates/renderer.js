...
module.exports = {
  ...
  paragraph(text) {
    ...
    return html`<p class="md-p">${icon}${text}</p>`;
  },
  list(body) {
    return html`<ol class="md-list">${body}</ol>`;
  },
  listitem(text) {
    return html`<li class="md-listitem">${text}</li>`;
  },
  codespan(code) {
    return html`<code class="md-codespan">${code}</code>`;
  },
  code(code, info) {
    [
      _,
  ...
  tablecell(content, flags) {
    if (flags.header) {
      return html`<th class="md-th">${content}</th>`;
    }
    return html`<td class="md-td">${content}</td>`;
  },
  table(header, body) {
    return html`<div class="md-table-wrapper">
