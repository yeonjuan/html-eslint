...
export interface AngularRuleListener {
...
export interface RuleFixer {
  insertTextAfter(nodeOrToken: Node, text: string): RuleFix;

  insertTextAfterRange(range: eslint.AST.Range, text: string): RuleFix;

  insertTextBefore(nodeOrToken: Node, text: string): RuleFix;

  insertTextBeforeRange(range: eslint.AST.Range, text: string): RuleFix;

  remove(nodeOrToken: Node): RuleFix;

  removeRange(range: eslint.AST.Range): RuleFix;

  replaceText(nodeOrToken: Node, text: string): RuleFix;
...
export interface RuleModule<Options extends unknown[]> {
  create(context: Context<Options>): AngularRuleListener;
