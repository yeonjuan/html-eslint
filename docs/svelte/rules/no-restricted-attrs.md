---
title: svelte/no-restricted-attrs
description: Disallow specified attributes in Svelte components based on custom configuration.
---

# no-restricted-attrs

This rule disallows the use of specified attributes in Svelte components.
...
export default [
  {
    ...
    rules: {
      "@html-eslint/svelte/no-restricted-attrs": [
        "error",
        {
          tagPatterns: ["^div$", "^img$"],
          attrPatterns: ["data-.*"],
