...
This rule disallows HTML attributes that have no effect in their context in Svelte components.
...
export default [
  {
    ...
    rules: {
      "@html-eslint/svelte/no-ineffective-attrs": "error",
    },
  },
];
```
...
**Note**: This rule only checks lowercase HTML elements (e.g., `<input>`, `<script>`). PascalCase Svelte components (e.g., `<Input>`) and custom elements with hyphens (e.g., `<my-input>`) are ignored.
...
<!-- Svelte expressions are skipped (dynamic values) -->
