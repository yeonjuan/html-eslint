...
const RULE_META = Object.entries(ALL_RULES)
...
function genSideBarsJson () {
  ...
  RULE_META.forEach(({name, meta}) => ruleDocIds[meta.docs.category].push(`rules/${name}`));
    
  BASE.docs.Rules.push(...Object.entries(ruleDocIds).map(([label, ids]) => createSubCatory(label, ids)));
...
function prependDocMeta () {
  const docPaths = fs.readdirSync(RULE_DOCS_DIR).map(name => path.resolve(RULE_DOCS_DIR, name));
...
function genAllRules () {
  function createAllRulesContent ({
    bestPractice,
    SEO,
    accessibility,
    styles
...
`;
  ...
  function createTableRowContent ({name, meta}) {
    return `| [@html-eslint/${name}](rules/${name}) | ${meta.docs.description} | ${meta.docs.recommended ? "⭐" : ""}${meta.fixable ? "🔧" : ''} |`
  } 
...
${rows}`
  ...
  const bestPractice = RULE_META
    .filter(({meta}) => meta.docs.category === RULE_CATEGORY.BEST_PRACTICE)
    .map(createTableRowContent).join('\n');
  const SEO = RULE_META
  .filter(({meta}) => meta.docs.category === RULE_CATEGORY.SEO)
  .map(createTableRowContent).join('\n');
  const accessibility = RULE_META
  .filter(({meta}) => meta.docs.category === RULE_CATEGORY.ACCESSIBILITY)
  .map(createTableRowContent).join('\n');
  const styles = RULE_META
    .filter(({meta}) => meta.docs.category === RULE_CATEGORY.STYLE)
    .map(createTableRowContent).join('\n');

  fs.writeFileSync(ALL_RULES_DOC_PATH, createAllRulesContent({
